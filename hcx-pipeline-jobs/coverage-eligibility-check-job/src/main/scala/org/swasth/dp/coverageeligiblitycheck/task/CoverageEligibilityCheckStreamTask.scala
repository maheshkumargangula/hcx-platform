  ...
  def process(): Unit = {
    ...
    val enrichedStream =
      env.addSource(kafkaConsumer, config.eligibilityCheckConsumer)
        ...
        .rebalance()
        .process(new ContextEnrichmentFunction(config)).name(config.contextEnrichmentFn)
        .uid(config.contextEnrichmentFn).setParallelism(config.downstreamOperatorsParallelism)


    val eventStream = enrichedStream.getSideOutput(config.enrichedOutputTag)
      .process(new CoverageEligibilityProcessFunction(config)).name(config.coverageEligibilityFn)
